...
class	MaxSizeNegative : public std::exception { public : const char * what() const throw() { return "\033[31merror\033[0m: client_max_body_size has to be positive."; } };
...
class	InvalidParameterReturnCode : public std::exception { private: char _msg[MSG_SIZE]; public: InvalidParameterReturnCode( const char * s ) { std::snprintf(_msg, sizeof(_msg), "\033[31merror\033[0m: `%s`: This code is not working for redirect.", s); } const char * what() const throw() { return _msg; } };
